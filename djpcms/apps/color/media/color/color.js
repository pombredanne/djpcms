(function (Raphael) {
    Raphael.colorwheel = function (element, size, initcolor) {
        return new ColorWheel(element, size, initcolor);
    };
    var pi = Math.PI,
        doc = document,
        win = window,
        ColorWheel = function (element, size, initcolor) {
            size = size || 200;
            var w3 = 3 * size / 200,
                w1 = size / 200,
                fi = 1.6180339887,
            ...
            var r = Raphael(element, size, size),
    ...
    $.djpcms.decorator({
        ...
        decorate: function($this, config) {
            ...
            $(opts.selector, $this).each(function() {
                var el = $(this),
                    val = el.val(),
                    picker = $.djpcms.raphael_picker;
                
                if(picker === undefined) {
                    var element = $('<div class="color-picker wheel"></div>').hide().appendTo($('body')),
                        picker = {'container': element,
                                  'current': null,
                                  'picker': Raphael.colorwheel(element[0], opts.size, "#eee")};
                    $.djpcms.raphael_picker = picker;
                    picker.picker.onchange = function(clr) {
                        picker.current.css({'background':clr});
                    };
                }
                if(val) {
                ...
                el.click(function() {
                    var el = $(this),
                        ...
                        pos = widget.position(),
                        picker = $.djpcms.raphael_picker;
                    picker.current = widget;
                    picker.container.css({'top':pos.top,'left':pos.left+width}).show();
                })
            });
        }
    });
