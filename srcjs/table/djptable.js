    ...
    if ($.fn.dataTable) {
        ...
        $.fn.dataTableExt.oApi.fnSetFilteringDelay = function (oSettings, iDelay) {
            ...
            this.each(function (i) {
                ...
                var $this = $(this),
                    container = $this.closest('div.data-table'),
                    oTimerId = null,
                    sPreviousSearch = null,
                    search_control = that.fnSettings().aanFeatures.f,
                    search = $('input', search_control).addClass('input-filter'),
                    controls = $('.input-filter', container);

                function server_request (input) {
                    var sPreviousSearch = input.data('sPreviousSearch'),
                        current;
                    // Chrome has issues when accessing the val jQuery method on inputs
                    if (input.is('input')) {
                        current = input[0].value;
                    } else {
                        current = input.val();
                    }
                    if (sPreviousSearch === undefined || sPreviousSearch !== current) {
                        window.clearTimeout(input.data('oTimerId'));
                        input.data('sPreviousSearch', current);
                ...
                //
                controls.unbind('keyup').bind('keyup', function (e) {
                    server_request($(this));
                });
                ...
                return function (value) {
                    if (value) {
                        ...
                        if (fields) {
                            ...
                            //cols.hide();
                            //selected.show();
                            $.each($.djpcms.datatable.fnDrawCallbacks, function (i, func) {
                                func(oTable);
                            });
                        }
                    }
