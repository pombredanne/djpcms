...
from djpcms.apps.contactus import HtmlContactForm
...
class TestFormValidation(test.TestCase):
    ...
    def testContantForm(self):
        d = HtmlContactForm(data={'body':'blabla'}, send_message=dummy_send)
        ...
        d = HtmlContactForm(data={'body':'blabla','name':''},
                            send_message=dummy_send)
        self.assertFalse(d.is_valid())
        self.assertEqual(len(d.errors),2)
        self.assertTrue('name' in d.errors)
        d = HtmlContactForm(data={'body':'blabla','name':'pippo',
